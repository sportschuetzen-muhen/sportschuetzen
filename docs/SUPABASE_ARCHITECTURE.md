...
- **Architektur & Komponenten:**
  ...
  - **Frontend-Integration & UI/UX:**
    ...
    - **Cache-Busting (`?v=20260926_5`):** Aktualisierung aller Script-Referenzen in `vorstand/index.html` (`auth.js?v=20260926_5`, `main.js?v=20260926_3`, `logins-actions.js?v=20260926_2`).
  - **Login UI/UX & Server-Härtung:**
    ...
    - `vorstand/js/main.js`: **Behebung des ausgegrauten Bildschirms nach Login:** Entfernung des blockierenden automatischen Aufrufs von `#welcome-security-modal` in `showApp()`; automatische und rückstandslose Bereinigung verwaister `.modal-backdrop`-Elemente sowie Entfernen von `modal-open` auf `document.body` bei jedem App-Eintritt.
    - `vorstand/js/main.js`: **Sidebar-Benutzeranzeige & Mobile Header:** Dynamische Aktualisierung von `#user-info` und `#user-badge-mobile` mit Namen und Rollen-Badge des authentifizierten Benutzers (Ablösung des statischen Rumpftexts «Nicht angemeldet»).
    - `vorstand/js/main.js`: **Präsenz-Tracker & Multi-User-Erkennung:** Sichere Namensauflösung aus den aggregierten Supabase-Objekten in `updatePresenceUI()` (Beseitigung von `Online: [object Object]`); automatisches Ausfiltern des aktuellen Benutzers zur korrekten Erkennung des Status *«Allein online»*.
    ...
    - `vorstand/js/logins/`: **Behebung der Profilerstellung & Auth-Aktivierung:**
      - **Sofortige Anzeige ohne Reload:** Robuste Modal-Steuerung per `bootstrap.Modal.getOrCreateInstance`, automatische Bereinigung verwaister `.modal-backdrop`-Elemente und Rücksetzen von Suchfiltern, sodass neu angelegte Admins sofort nach dem Speichern in der Tabelle sichtbar sind.
      - **Vorstandsfunktion & Auto-Fill:** Automatische Funktionsübernahme beim Verknüpfen von Vereinsmitgliedern aus `window._mglFunktionenCache`, `OfficialFunctionCategory` oder Vorstands-Stammdaten; Datalist für Standardfunktionen (`Präsident`, `Mitgliederverwalter`, `Kassier`, `Aktuar` etc.) und Umbenennung des verwirrenden Labels «Rolle extern» zu «Vorstandsfunktion».
      - **Supabase-Auth-Provisionierung:** Direkte Auth-Registrierung via `supabase.auth.signUp` bei Passworteingabe; Bereitstellung des Schnellaktions-Buttons *«Einladen»* in der Tabelle für ausstehende Auth-Konten via `loginsSendInvite` (sendet Aktivierungs-Link über GoTrue SMTP und auditiert den Vorgang direkt in `public.mail_logs`).
      - **Live-Daten:** Verknüpfung und Aktivierung von Vorstandskonto `b.augsburger` (`beat.augsburger@brugg.ch`, Funktion *«Mitgliederverwalter»*) mit GoTrue `auth.users` und `public.user_roles`.
    - **Cache-Busting (`?v=20260926_6`):** Aktualisierung der Logins-Skript-Referenzen in `vorstand/index.html`.
