...
| **5** | **Anlässe & Umfragen (Eventplaner)** | Eigenständige Supabase-Migration des RSVP- und Umfragen-Moduls (`poll_events`, `poll_responses`, `poll_views`, `poll_responses_log`, `07_eventplaner_module.sql`); Beibehaltung der Modultrennung; Dual-Write zum Google Sheet (`1lN180...`). *(Zukünftige Erweiterung: Dynamischer Fragen-Baukasten / Variante B im Backlog vorgemerkt, nicht jetzt)* | Konsolidierung des RSVP-Systems mit Dual-Write zu Google Sheet. (Mittel) | ✅ **Abgeschlossen & im Testbetrieb** |
...
| **7** | **Inventar** | Vereinsinventar, Ausleihe und Rückgabe (`08_inventory_module.sql`, Dual-Write zu Google Sheet) | Relativ unabhängig von anderen Modulen; direkte Supabase-First-Anbindung & 1-Klick-Sheet-Import. (Niedrig) | ✅ **Abgeschlossen & im Testbetrieb** |
